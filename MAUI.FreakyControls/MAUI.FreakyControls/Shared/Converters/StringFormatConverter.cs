        ...
        try
        {
            if (targetType != typeof(string))
                throw new ArgumentException(
                    $"Converter can only convert to string, {targetType} is not a supported target type",
                    nameof(targetType));
            ...
            if (!(parameter is string format))
                throw new ArgumentException(
                    $"Converter parameter must be a string, {parameter.GetType()} is not supported", nameof(parameter));
